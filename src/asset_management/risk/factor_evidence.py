...
def publish_factor_risk_evidence(*, store: ImmutableDatasetStore, assessment: FactorRiskAssessment,
    ...
    Parent existence and matching licenses preserve references, not proof that
    an estimator consumed them. This mechanism-only record never passes D2 until
    a separate canonical estimator contract can replay complete return inputs.
    """
    ...
    if assessment.estimation_version != policy.estimation_version:
        raise DataQualityError("FACTOR_RISK_POLICY_VERSION_MISMATCH")
    ...
    body = {"assessment": assessment.payload(), "specific_risk_policy": specific_risk_policy_payload(policy),
            "validation_scope": "MECHANISM_ONLY", "estimation_lineage_verified": False,
            "covariance": [[str(value) for value in row] for row in assessment.covariance]}
    ...
    return store.write(
        ...
        schema_version="factor-specific-risk-evidence@2", retrieved_at=available_at,
