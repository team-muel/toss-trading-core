...
from asset_management.compatibility.account_evidence.ledger import AccountLedger
...
class AccountEvidenceReplayResult:
    source_run_id: str
    replay_run_id: str
    account_seq: str
    raw_rows: int
...
def replay_account_evidence_run(
    *,
    ...
    """Rebuild normalized Account evidence rows from stored raw broker responses.
    ...
    if not source_path.is_file():
        raise FileNotFoundError(f"source Account evidence database not found: {source_path}")
    ...
    try:
        ...
        if run is None:
            raise ValueError("no matching complete Account evidence snapshot run")
        ...
        for row in raw_rows:
            ...
            if path == "/api/v1/accounts":
                ...
                raise ValueError(f"unsupported Account evidence broker endpoint in replay: {endpoint}")
        ...
        return AccountEvidenceReplayResult(
