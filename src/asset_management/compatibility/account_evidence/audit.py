...
from asset_management.toss.runtime import JsonlLogger
...
class AccountEvidenceAuditResult:
    ok: bool
    lines: list[str]
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Audit whether the account evidence account-state snapshot is complete.",
    )
    parser.add_argument(
        "--db",
...
def _schema_failure(db_path: str | Path) -> str | None:
    ...
    try:
        ...
        if missing:
            return f"incompatible_account_evidence_schema:missing_tables={','.join(missing)}"
        run_columns = {
        ...
        if missing_columns:
            return f"incompatible_account_evidence_schema:missing_snapshot_run_columns={','.join(missing_columns)}"
    finally:
        conn.close()
    return None


def audit_account_evidence_db(
    *,
    db_path: str | Path,
...
) -> AccountEvidenceAuditResult:
    failures: list[str] = []
    lines = ["account_evidence_audit=running", f"profile={profile}"]
    ...
    if schema_failure is not None:
        return AccountEvidenceAuditResult(
            ok=False,
    ...
    try:
        ...
        if failures:
            return AccountEvidenceAuditResult(
                ok=False,
                lines=["account_evidence_audit=failed", *lines[1:], *[f"failure={x}" for x in failures]],
            )
        return AccountEvidenceAuditResult(ok=True, lines=["account_evidence_audit=ok", *lines[1:]])
    finally:
        ledger.close()
...
def main(argv: list[str] | None = None) -> int:
    ...
    logger.emit("account_evidence_audit_start", db=args.db, profile=args.profile)
    ...
    logger.emit(
        "account_evidence_audit_ok" if result.ok else "account_evidence_audit_failed",
        profile=args.profile,
        lines=result.lines,
    )
