"""Outer integration boundary from research alpha evidence to production Signal candidates.

This module is deliberately placed in orchestration rather than the investment core:
`alpha_management` is a sibling research package and core `asset_management` modules
must not import it. The bridge consumes research *raw expression scores*, not
simulated positions, forecasts, expected returns, portfolio weights, or order intents.
"""
