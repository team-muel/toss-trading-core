...
def require_horizon_alignment(signals: Iterable[SignalValidity]) -> SignalValidity:
    ...
    for item in values[1:]:
        if (item.forecast_horizon != first.forecast_horizon or
                item.holding_horizon != first.holding_horizon):
            raise DataQualityError("SIGNAL_HORIZON_MISMATCH")
        # Matching horizon numbers are not sufficient: expiry and decay semantics
        # are part of the same validity contract and must also align exactly.
        if item != first:
