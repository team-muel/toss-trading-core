"""Sparse replay journal for point-in-time research input snapshots."""
...
class InputJournal:
    """Retain only the previous snapshot and bounded deltas."""
    def __init__(self):
        self.previous = {}
        self.deltas = []
...
def iter_snapshots(deltas) -> Iterator[dict]:
    """Replay independent snapshots without retaining the complete history."""
    current = {}
    for delta in deltas:
