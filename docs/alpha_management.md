...
| --- | --- | --- |
| Alpha expressions, operators, simulation, metrics | `alpha_management` | No compatibility runtime is packaged |
...
| Research-to-decision proposal | Governed bridge (not implemented) | Proposals must pass canonical risk and execution gates |
...
`alpha_management` is the sole alpha-expression implementation. Provider data
must be normalized through `asset_management` before alpha-management research
consumes it; no compatibility datafield or execution adapter is packaged.
