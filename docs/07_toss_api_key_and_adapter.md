...
| `orders.list_closed()` | `GET /api/v1/orders` | OpenAPI 1.2.14 계약에 따라 7일 중첩 window로 정기 대사 |
...
- OPEN 주문 주기 대사
- CLOSED 목록은 OpenAPI 1.2.14가 cursor pagination을 명시합니다. 기본 runner가 7일 중첩 window로 수집하며, 범위를 넓히는 복구 실행만 명시적으로 수행합니다.
...
하나라도 빠지면 read-only canonical adapter 이외의 사용을 허용하지 않습니다.
