...
from asset_management.decisions.governor import SOFT_REDUCTIONS
...
def source_intent():
    ...
    decision = RiskGovernor(policy).decide(RiskInputs(
        runtime_run_id="run:1", portfolio_target_id="target:1", portfolio_target_hash="target-hash",
        policy_version="risk@1", as_of_utc=NOW.isoformat(), evidence_ids=("account:1", "risk:1"),
    )).authorize()
    return OrderIntent("run:1", "risk@1", "target:1", "target-hash", decision,
                       (TargetWeight("SPY", D(".5"), D(".1")),), ("rebalance",))


def kwargs(**changes):
    values = dict(source=source_intent(), nav=D(1000),
                  quotes={"SPY": ExecutableQuote(D("100.03"), NOW, NOW + timedelta(minutes=1), True)},
                  current_quantities={"SPY": D(".5")}, open_order_quantities={"SPY": D(2)},
...
def test_missing_rule_or_unapproved_risk_binding_fails_closed():
    ...
    approved = source_intent().risk_authorization
    with pytest.raises(InvariantViolation, match="different portfolio target"):
        OrderIntent("run:1", "risk@1", "other", "target-hash", approved,
                    (TargetWeight("SPY", D(".5"), D(".1")),), ())
...
def test_tampered_plan_or_sub_minimum_delta_is_rejected_or_skipped():
    ...
    assert plan_order_intents(**kwargs(current_quantities={"SPY": D(2)})) == ()
