...
def components(kind, point=D(".01"), uncertainty=D(".001"), confidence=D(".8")):
    return {name: ExpectedReturnComponent(name,point,uncertainty,confidence,(f"feature:{name}",),252) for name in COMPONENTS[kind]}
...
def required(rate=D(".02")):
    ...
    return capm_required_return(instrument_id="X",risk_free_rate=rate,beta=beta,market_risk_premium=D(".05"),horizon=252,as_of=NOW)
