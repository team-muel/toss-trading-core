...
INSTRUMENTS=("RISK","CASH")
def optimizer_authority(risk_aversion=D(1),active_risk_aversion=D(0),objective=MandateObjective.ABSOLUTE_WEALTH):
    ...
    mandate=InvestorMandate("household","1",objective,"USD","USD",WealthConvention.NOMINAL,63,21,21,"SPY@1",None,"CASH@1",preference,NOW,NOW+timedelta(days=1))
...
def target(weights=(D(".5"),D(".5")),stage="TEST"):
    return PortfolioTarget(INSTRUMENTS,weights,stage)
...
def test_objective_has_half_variance_and_no_double_counting():
    returns=OptimizationReturnInput(OptimizationReturnMode.ABSOLUTE,("RISK",),(D(".1"),))
    value=objective((D(1),),returns,((D(".04"),),),(D(1),),risk_aversion=D(2),linear_cost=(D(0),),impact_cost=(D(0),))
    assert value==D(".06")
    with pytest.raises(DataQualityError): objective((D(1),),returns,((D(".04"),),),(D(1),),risk_aversion=D(2),linear_cost=(D(0),),impact_cost=(D(0),),turnover_penalty=D(".1"))

def _optimizer_inputs(return_input=None):
    return dict(instruments=INSTRUMENTS,return_input=return_input or absolute_return_input(),
        covariance=((D(".04"),D(0)),(D(0),D(".001"))),covariance_instruments=INSTRUMENTS,
        current=(D(".2"),D(".8")),cash_instrument="CASH",max_single_weight=D(".9"),min_cash_weight=D(".1"),
        risk_aversion=D(1),linear_cost=(D(".001"),D(".001")),impact_cost=(D(".01"),D(".01")))

def test_optimizer_moves_toward_forecast_total_return_but_returns_weights_only():
    result=optimize_weights(**_optimizer_inputs(),iterations=200,**optimizer_authority())
    assert result.weights[0]>D(".2") and sum(result.weights)==D(1)
    assert not ({"order","side"}&set(result.__dataclass_fields__))
    nearby=optimize_weights(**_optimizer_inputs(absolute_return_input((D(".1001"),D(".01")))),iterations=200,**optimizer_authority())
    assert abs(nearby.weights[0]-result.weights[0])<D(".01")
    with pytest.raises(DataQualityError,match="OPTIMIZER_INPUT_INVALID"):
        optimize_weights(**(_optimizer_inputs() | {"covariance_instruments":("CASH","RISK")}),iterations=5,**optimizer_authority())

def test_capital_basis_return_semantics_and_stability_evidence_fail_closed():
    capital=CapitalAllocation(D(100),D(10),D(5),D(".2"))
    assert capital.investable_capital==D(95) and capital.risk_capital==D(19)
    absolute=absolute_return_input()
    active=OptimizationReturnInput(OptimizationReturnMode.BENCHMARK_ACTIVE,INSTRUMENTS,(D(".10"),D(".01")),benchmark_weights=(D(".5"),D(".5")))
    assert absolute.expected_return((D(".6"),D(".4")))==D(".064")
    assert active.expected_return((D(".6"),D(".4")))==D(".009")
    with pytest.raises(DataQualityError,match="MODEL_ALPHA_ALIGNMENT"):
        OptimizationReturnInput(OptimizationReturnMode.MODEL_RELATIVE_ALPHA,INSTRUMENTS,(D(".1"),D(".01")),benchmark_weights=(D(".5"),D(".5")),model_relative_alpha=(D(".01"),D(0)))
    stability_policy=OptimizationStabilityPolicy("CASH",D(".1"),D(".1"),D(".9"))
    evidence=OptimizationStabilityEvidence(target((D(".5"),D(".5"))),target((D(".55"),D(".45"))),
        target((D(".56"),D(".44"))),target((D(".54"),D(".46"))),stability_policy)
    assert evidence.violations()==()
    unstable=OptimizationStabilityEvidence(target((D(".5"),D(".5"))),target((D(".55"),D(".45"))),
        target((D(".8"),D(".2"))),target((D(".54"),D(".46"))),stability_policy)
    with pytest.raises(DataQualityError,match="WEIGHT_JUMP"):
        unstable.require_stable()

def test_active_and_model_alpha_modes_require_a_matching_benchmark_mandate():
    active=OptimizationReturnInput(OptimizationReturnMode.BENCHMARK_ACTIVE,INSTRUMENTS,(D(".10"),D(".01")),benchmark_weights=(D(".2"),D(".8")))
    inputs=_optimizer_inputs(active) | {"active_risk_aversion":D(1),"iterations":50}
    result=optimize_weights(**inputs,**optimizer_authority(active_risk_aversion=D(1),objective=MandateObjective.BENCHMARK_RELATIVE))
    assert result.weights[0]>D(".2")
    with pytest.raises(InvariantViolation,match="OBJECTIVE_MANDATE_MISMATCH"):
        optimize_weights(**inputs,**optimizer_authority(active_risk_aversion=D(1)))
    model=OptimizationReturnInput(OptimizationReturnMode.MODEL_RELATIVE_ALPHA,INSTRUMENTS,(D(".10"),D(".01")),
        benchmark_weights=(D(".2"),D(".8")),model_relative_alpha=(D(".02"),D(0)),pricing_baseline_version="pricing-baseline@1",factor_neutrality_verified=True)
    assert optimize_weights(**(inputs | {"return_input":model}),**optimizer_authority(
        active_risk_aversion=D(1),objective=MandateObjective.BENCHMARK_RELATIVE)).weights[0]>D(".2")
