...
def test_objective_has_half_variance_and_no_double_counting():
    returns=OptimizationReturnInput(OptimizationReturnMode.ABSOLUTE,(D(".1"),))
...
def test_optimizer_moves_toward_forecast_total_return_but_returns_weights_only():
    result=optimize_weights(instruments=("RISK","CASH"),return_input=absolute_return_input(),covariance=((D(".04"),D(0)),(D(0),D(".001"))),current=(D(".2"),D(".8")),cash_instrument="CASH",max_single_weight=D(".9"),min_cash_weight=D(".1"),risk_aversion=D(1),linear_cost=(D(".001"),D(".001")),impact_cost=(D(".01"),D(".01")),iterations=200,**optimizer_authority())
    assert result.weights[0]>D(".2") and sum(result.weights)==D(1)
    assert not ({"order","side"}&set(result.__dataclass_fields__))
    nearby=optimize_weights(instruments=("RISK","CASH"),return_input=absolute_return_input((D(".1001"),D(".01"))),covariance=((D(".04"),D(0)),(D(0),D(".001"))),current=(D(".2"),D(".8")),cash_instrument="CASH",max_single_weight=D(".9"),min_cash_weight=D(".1"),risk_aversion=D(1),linear_cost=(D(".001"),D(".001")),impact_cost=(D(".01"),D(".01")),iterations=200,**optimizer_authority())
...
def test_capital_basis_return_semantics_and_stability_evidence_fail_closed():
    ...
    active=OptimizationReturnInput(OptimizationReturnMode.BENCHMARK_ACTIVE,(D(".10"),D(".01")),(D(".5"),D(".5")))
    ...
    with pytest.raises(DataQualityError,match="MODEL_ALPHA_ALIGNMENT"):
        OptimizationReturnInput(OptimizationReturnMode.MODEL_RELATIVE_ALPHA,(D(".1"),D(".01")),(D(".5"),D(".5")),(D(".01"),D(0)))
    policy=OptimizationStabilityPolicy("CASH",D(".1"),D(".1"),D(".9"))
    evidence=OptimizationStabilityEvidence(target((D(".5"),D(".5"))),target((D(".55"),D(".45"))),
        target((D(".56"),D(".44"))),target((D(".54"),D(".46"))),policy)
    assert evidence.violations()==()
    unstable=OptimizationStabilityEvidence(target((D(".5"),D(".5"))),target((D(".55"),D(".45"))),
        target((D(".8"),D(".2"))),target((D(".54"),D(".46"))),policy)
...
def test_active_and_model_alpha_modes_require_a_matching_benchmark_mandate():
    active=OptimizationReturnInput(OptimizationReturnMode.BENCHMARK_ACTIVE,(D(".10"),D(".01")),(D(".2"),D(".8")))
    inputs=dict(instruments=("RISK","CASH"),return_input=active,covariance=((D(".04"),D(0)),(D(0),D(".001"))),
        current=(D(".2"),D(".8")),cash_instrument="CASH",max_single_weight=D(".9"),min_cash_weight=D(".1"),
        risk_aversion=D(1),active_risk_aversion=D(1),linear_cost=(D(".001"),D(".001")),impact_cost=(D(".01"),D(".01")),iterations=50)
    ...
    model=OptimizationReturnInput(OptimizationReturnMode.MODEL_RELATIVE_ALPHA,(D(".10"),D(".01")),
        (D(".2"),D(".8")),(D(".02"),D(0)),"pricing-baseline@1",True)
