...
def test_optimizer_moves_toward_alpha_but_returns_weights_only():
    result=optimize_weights(instruments=("RISK","CASH"),alpha=(D(".10"),D(".01")),covariance=((D(".04"),D(0)),(D(0),D(".001"))),current=(D(".2"),D(".8")),cash_instrument="CASH",max_single_weight=D(".9"),min_cash_weight=D(".1"),risk_aversion=D(1),linear_cost=(D(".001"),D(".001")),impact_cost=(D(".01"),D(".01")),iterations=200)
    assert result.weights[0]>D(".2") and sum(result.weights)==D(1)
    assert not ({"order","side"}&set(result.__dataclass_fields__))
    nearby=optimize_weights(instruments=("RISK","CASH"),alpha=(D(".1001"),D(".01")),covariance=((D(".04"),D(0)),(D(0),D(".001"))),current=(D(".2"),D(".8")),cash_instrument="CASH",max_single_weight=D(".9"),min_cash_weight=D(".1"),risk_aversion=D(1),linear_cost=(D(".001"),D(".001")),impact_cost=(D(".01"),D(".01")),iterations=200)
